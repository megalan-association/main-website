...
import TempEvent from "../../../public/temp_event.png"
import TempEvent2 from "../../../public/temp_event2.jpg"
  ...
  return (
    <div className="flex h-full flex-col p-4 items-center">
      ...
      <div className="gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 h-fit p-5">
        {list.map((item, index) => (
          // TODO: REMOVE TEMP
          <EventCard index={index} title={item.title} description={item.description} image={index % 2 ? TempEvent.src : TempEvent2.src} link={item.link ? item.link : null} />
        ))}
      </div>
    </div>
  );
